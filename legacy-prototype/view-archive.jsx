// view-archive.jsx — World archive + Story seeds pool + Conflicts pool
...
const ArchiveView = ({ world, savedSettings, savedIssues = [], onOpenDetail, onOpenIssues, onBackToWorkbench }) => {
  ...
  const counts = useMemoA(() => {
    const c = { all: savedSettings.length };
  ...
  return (
    <div className="view-scroll" style={{ flex: 1, minHeight: 0, display: "flex", flexDirection: "column" }}>
      ...
      <div style={{
        ...
        <div className="flex"/>
        <input className="input" placeholder="搜索档案…" value={q} onChange={e => setQ(e.target.value)}
              ...
              return (
                <button key={s.id} className="card hover" onClick={() => onOpenDetail(s)}
                  ...
                  <div className="row gap-2" style={{ alignItems: "flex-start" }}>
                    ...
                    <div className="flex"/>
                    {issueCount > 0 && (
                      <span onClick={(e) => { e.stopPropagation(); onOpenIssues && onOpenIssues(s.id); }}
                  ...
                  {s.relations && (
                    <div className="row gap-2" style={{ flexWrap: "wrap" }}>
                      {s.relations.slice(0, 3).map(r => <span key={r} className="tag plain" style={{ fontSize: 10 }}>↳ {r}</span>)}
                    </div>
                  )}
...
const ArchiveEmpty = ({ hasAny, onBackToWorkbench }) => (
  <div style={{
...
// ────────── Story Seeds Pool ──────────
const SeedsView = ({ world, savedSeeds, savedConflicts = [], onOpenDetail, onBackToWorkbench, onJumpToConflict }) => {
  const [filter, setFilter] = useStateA("all");
  const [q, setQ] = useStateA("");

  // "high potential" heuristic: 3+ open questions = more dramatic surface
  const filtered = useMemoA(() => {
    return savedSeeds.filter(s => {
  ...
  const counts = useMemoA(() => ({
    ...
    high: savedSeeds.filter(s => (s.questions?.length || 0) >= 3).length,
    draft: savedSeeds.filter(s => (s.questions?.length || 0) < 3).length,
  }), [savedSeeds]);
  ...
  return (
    <div className="view-scroll" style={{ flex: 1, minHeight: 0 }}>
      ...
      <div style={{
        ...
        <div className="flex"/>
        <input className="input" placeholder="搜索种子…" value={q} onChange={e => setQ(e.target.value)}
          style={{ width: 200, height: 26, fontSize: 12 }}/>
      </div>
      ...
      <div style={{ padding: "20px 32px 40px" }}>
        {savedSeeds.length === 0 ? (
          ...
          <div className="col" style={{ gap: 12 }}>
            {filtered.map((s, idx) => {
              const parent = s.parentConflict ? savedConflicts.find(c => c.id === s.parentConflict) : null;
              return (
                <button key={s.id} className="card hover" onClick={() => onOpenDetail(s)}
                  style={{
                  ...
                  <div style={{ flex: 1, padding: "14px 18px" }}>
                    <div className="row gap-2" style={{ marginBottom: 6, flexWrap: "wrap" }}>
                      ...
                      {parent && (
                        <span onClick={(e) => { e.stopPropagation(); onJumpToConflict && onJumpToConflict(parent); }}
...
const SeedsEmpty = ({ onBackToWorkbench }) => (
  <div style={{
    padding: 60, textAlign: "center", color: "var(--fg-2)",
    border: "1px dashed var(--border)", borderRadius: 8, margin: "0 auto", maxWidth: 540,
  }}>
...
const ConflictsView = ({ world, savedConflicts, savedSeeds = [], onOpenDetail, onBackToWorkbench }) => {
  const [cat, setCat] = useStateA("all");
  const [q, setQ] = useStateA("");
  ...
  const rawCat = (c) => (c.category || "未分类").split("·")[0].trim();
  ...
  const filtered = useMemoA(() => {
    return savedConflicts.filter(c => {
  ...
  const counts = useMemoA(() => {
    const c = { all: savedConflicts.length };
    ...
    <div className="view-scroll" style={{ flex: 1, minHeight: 0 }}>
      ...
      {savedConflicts.length > 0 && (
        <div style={{
          ...
          <div className="flex"/>
          <input className="input" placeholder="搜索冲突…" value={q} onChange={e => setQ(e.target.value)}
            style={{ width: 200, height: 26, fontSize: 12 }}/>
        </div>
      )}
      ...
      <div style={{ padding: "20px 32px 40px" }}>
        {savedConflicts.length === 0 ? (
          ...
          <div className="col" style={{ gap: 12 }}>
            {filtered.map(c => {
              const derivedCount = savedSeeds.filter(s => s.parentConflict === c.id).length;
                  ...
                  <div style={{ padding: "16px 18px 12px", display: "flex", flexDirection: "column", gap: 8 }}>
                    ...
                    {c.related && (
                      <div className="row gap-2" style={{ flexWrap: "wrap", marginTop: 4 }}>
                        {c.related.map(r => <span key={r} className="tag">↳ {r}</span>)}
                      </div>
                    )}
                  </div>
                  <div style={{
...
Object.assign(window, { ArchiveView, SeedsView, ConflictsView });
