// view-worlds.jsx — Worlds list page + Create flow

const { useState, useEffect, useMemo, useRef } = React;

// ────────── World card ──────────
const WorldCard = ({ world, onOpen, onDelete, onDuplicate }) => {
  ...
  return (
    <div className="card hover" onClick={(e) => { if (e.target.closest('[data-menu]')) return; onOpen(world.id); }}
      style={{
      ...
      <div style={{ padding: "14px 16px 12px", flex: 1, display: "flex", flexDirection: "column", gap: 8 }}>
            ...
            <div style={{ position: "relative" }} ref={menuRef}>
              ...
              {menuOpen && (
                <div onClick={(e) => e.stopPropagation()} style={{
                  ...
                  <button className="menu-item" onClick={() => { setMenuOpen(false); onDuplicate && onDuplicate(world.id); }}>
        ...
        <div className="row gap-2" style={{ flexWrap: "wrap" }}>
          {world.tags.map(t => <span key={t} className="tag">{t}</span>)}
        </div>
      </div>
      <div style={{
...
// ────────── Empty state card ──────────
const EmptyWorldCard = ({ inspirations, onPick }) => (
...
// ────────── Worlds list view ──────────
const WorldsView = ({ worlds, onOpen, onCreate, savedDraft, onContinueDraft, onDelete, onDuplicate, hideDraftFromList }) => {
  ...
  const filtered = useMemo(() => {
    return worlds.filter(w => {
      ...
      if (q && !w.name.includes(q) && !w.summary.includes(q) && !w.tags.some(t => t.includes(q))) return false;
      return true;
    });
    ...
    <div className="view-scroll" style={{ flex: 1, minHeight: 0 }}>
      <div className="page-head">
        <div className="col">
          ...
          <h1>我的世界</h1>
          <div className="sub">{worlds.length} 个世界 · {worlds.filter(w => w.status === "published").length} 个已公开 · {worlds.filter(w => w.hasUnsaved || w.hasUnpushed).length} 个有未处理改动</div>
        </div>
        <div className="row gap-2">
      ...
      <div style={{
        ...
        {[
          ...
          { id: "published", label: "已公开", n: worlds.filter(w => w.status === "published").length },
          { id: "draft", label: "草稿 / 未公开", n: worlds.filter(w => w.status !== "published").length },
          { id: "local", label: "Local", n: worlds.filter(w => w.mode === "local").length },
        ].map(f => (
      ...
      <div style={{
        ...
        {filtered.map(w => <WorldCard key={w.id} world={w} onOpen={onOpen} onDelete={onDelete} onDuplicate={onDuplicate}/>)}
        ...
        {!savedDraft && filtered.length > 0 && filter === "all" && <EmptyWorldCard inspirations={inspirations} onPick={s => onCreate(s)}/>}
      </div>
    </div>
  );
};
...
// ────────── Create World view ──────────
const CreateView = ({ initialInspiration, seedKey, onConfirm, onCancel }) => {
  ...
  const seed = MOCK.SEEDS[seedKey] || MOCK.SEEDS.memory;
      ...
      <div style={{ maxWidth: 720, margin: "0 auto", padding: "32px 32px 60px" }}>
        {step === "input" && (
          <div className="col gap-6" style={{ gap: 20 }}>
            <div>
              <label className="label">初始灵感 <span className="opt">必填</span></label>
              <textarea className="textarea" placeholder="一句话就够。例如：一个世界里，记忆可以被买卖。"
                value={inspiration} onChange={e => setInspiration(e.target.value)} rows={3}
                style={{ fontSize: "var(--t-15)", lineHeight: 1.6 }} autoFocus/>
            </div>
            <div className="row gap-3" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
              <div>
                <label className="label">世界名称 <span className="opt">可选 — Agent 会建议</span></label>
                <input className="input" placeholder="留空让 Agent 起名"
                  value={name} onChange={e => setName(e.target.value)}/>
              </div>
              <div>
                <label className="label">类型 <span className="opt">可选</span></label>
                <input className="input" placeholder="近未来 / 奇幻 / 蒸汽朋克…"
                  value={type} onChange={e => setType(e.target.value)}/>
              </div>
            </div>
            <div className="row gap-3" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
              <div>
                <label className="label">风格关键词 <span className="opt">可选</span></label>
                <input className="input" placeholder="冷静观察 · 制度细节 · 道德灰度"
                  value={styleKw} onChange={e => setStyleKw(e.target.value)}/>
              </div>
              <div>
                <label className="label">不想要的方向 <span className="opt">可选</span></label>
                <input className="input" placeholder="例如：爽文 / 单线主角 / 一键拯救世界"
...
const ConfirmCard = ({ seed, overrideName, overrideType, inspiration, onRegenerate, onConfirm }) => {
        ...
        <div className="col gap-3" style={{ gap: 14 }}>
          <div>
            ...
            <div className="row gap-2" style={{ marginTop: 6 }}>
              ...
              {seed.styles.map(s => <span key={s} className="tag">{s}</span>)}
            </div>
          </div>
...
Object.assign(window, { WorldsView, CreateView });
