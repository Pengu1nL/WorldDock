  ...
  return (
    <main style={{ minHeight: "100vh", background: "var(--bg)", color: "var(--fg)", padding: "56px min(8vw, 72px)" }}>
      ...
      <section style={{ marginTop: 24, display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: 16 }}>
        {["Creator Alpha", "Studio Beta", "Team Beta"].map((plan, index) => (
          <article key={plan} className="card" style={{ padding: 18 }}>
            <h2 className="title-font" style={{ marginTop: 0, fontSize: "var(--t-18)" }}>{plan}</h2>
            <div style={{ fontSize: 28, fontWeight: 700 }}>{index === 0 ? "免费试用" : "Beta 后开放"}</div>
            <p className="prose">{index === 0 ? "包含云端工作台、公开仓库和 Alpha 免费额度。" : "团队协作、模板库和高级治理会在 Beta 进入计划。"}</p>
            <button className="btn primary" onClick={() => trackProductEvent(PRODUCT_EVENTS.billingPlaceholderClicked, { plan })}>加入候补</button>
          </article>
        ))}
      </section>
