...
export function useTweaks(defaults: any): [any, (keyOrEdits: any, val?: any) => void] {
  ...
  return (
    <TweakRow label={label} value={`${value}${unit}`}>
      <input type="range" className="twk-slider" min={min} max={max} step={step}
             value={value} onChange={(e: any) => onChange(Number(e.target.value))} />
    </TweakRow>
  );
}
...
export function TweakRadio({ label, value, options, onChange }: any) {
  ...
  const fitLimits: Record<number, number> = { 2: 16, 3: 10 };
  const fitsAsSegments = maxLen <= (fitLimits[options.length] ?? 0);
  if (!fitsAsSegments) {
...
export function TweakSelect({ label, value, options, onChange }: any) {
  return (
    <TweakRow label={label}>
      <select className="twk-field" value={value} onChange={(e: any) => onChange(e.target.value)}>
        {options.map((o: any) => {
  ...
  return (
    <div className="twk-num">
      ...
      <input type="number" value={value} min={min} max={max} step={step}
             onChange={(e: any) => onChange(clamp(Number(e.target.value)))} />
...
function __twkIsLight(hex: any) {
  ...
  const x = h.length === 3 ? h.replace(/./g, (c: string) => c + c) : h.padEnd(6, '0');
        ...
        {options.map((o: any, i: number) => {
          ...
          return (
            <button key={i} type="button" className="twk-chip" role="radio"
              ...
              {sup.length > 0 && (
                <span>
                  {sup.map((c: any, j: number) => <i key={j} style={{ background: c }} />)}
                </span>
              )}
