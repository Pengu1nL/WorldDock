...
describe("billing endpoints", () => {
  ...
  it("grants initial credit on first balance read and returns ledger-backed usage", async () => {
    ...
    expect(usage.body.usage.entries).toHaveLength(1);
    expect(usage.body.usage.entries[0]).toMatchObject({
      type: "credit_granted",
      amountCents: 10000,
    });
  });
...
function createInMemoryBillingRepository() {
  ...
  const repository: BillingRepository = {
    ...
    async markFreeCreditGranted(accountId, grantedAt) {
